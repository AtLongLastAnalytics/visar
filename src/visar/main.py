...
from .helpers import docker_funcs as dof
from .helpers import helper_funcs as hf
from .helpers import osv_funcs as osv
...
def run_prerequisite_checks(repo_url: str, github_token: str) -> None:
    ...
    if not hf.validate_github_url(repo_url):
        hf.exit_with_error(VisarPrerequisiteError("Invalid GitHub repository URL"))

    logger.info("Verifying GitHub token...")
    if not hf.verify_github_token(github_token):
        hf.exit_with_error(VisarPrerequisiteError("Invalid GitHub token"))

    logger.info("Checking Docker is running...")
    if not dof.check_docker_isrunning():
        hf.exit_with_error(VisarDockerError("Docker is not running"))

    logger.info("Checking Docker image exists...")
    if not dof.check_dockerimage_exists(DOCKER_CONFIG["CONTAINER_NAME"]):
        hf.exit_with_error(VisarDockerError("Docker image does not exist locally"))
...
def execute_docker_commands(
    ...
    if not hf.retry_call(
        ...
        hf.exit_with_error(
            VisarDockerError("Failed to execute Docker command: vulnerabilities")
        )
...
def perform_data_transformation(repo_url: str, summary_filename: Path) -> Any:
    ...
    try:
        ...
        cleanup_temp_files()
        hf.exit_with_error(
            VisarDataError("Temp. file not found during data transformation")
        )
    except Exception as e:
        cleanup_temp_files()
        hf.exit_with_error(
            VisarDataError(f"Unexpected error during data transformation: {e}")
        )


def call_osv_api(vuln_ids: List[str]) -> List[Finding]:
    """
    Call the OSV API to retrieve vulnerability findings.

    This function uses the provided list of vulnerability IDs to call the OSV
    API to retrieve detail and severity information for each vulnerability,
    returning them as Finding objects. On failure, the process is terminated
    using hf.exit_with_error.

    Args:
        vuln_ids (List[str]): The vulnerability IDs to be looked up on the OSV
            API.

    Returns:
        List[Finding]: The vulnerability findings returned by the OSV API.
    """
    try:
        logger.info("Calling OSV API...")
        findings = hf.retry_call(
            osv.fetch_details,
            vuln_ids,
...
def scan_single_repository(repo_url: str, output_format: str) -> None:
    ...
    try:
        data_filename = hf.format_filename(repo_url)
    except ValueError as e:
        hf.exit_with_error(VisarPrerequisiteError(str(e)))
    ...
    findings = call_osv_api(vuln_ids)

    # 5. write the final output in the requested format
    write_output(data_filename, findings, output_format)
...
def main() -> None:
    ...
    if args.batch:
        ...
        try:
            ...
            hf.exit_with_error(
                VisarPrerequisiteError(f"Batch file not found: {args.batch}")
            )

        if not repo_urls:
            hf.exit_with_error(
                VisarPrerequisiteError("No valid repository URLs found in batch file.")
            )
